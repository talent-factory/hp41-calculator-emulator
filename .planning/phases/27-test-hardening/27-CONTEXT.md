...
- **D-27.15 (AMENDED 2026-05-15 after research): E2E launch mode — production build + `tauri-driver` + WebdriverIO (WebKitGTK on Ubuntu).** Build the Tauri app with `just gui-build` (release binary), then drive it via `tauri-driver` (listens on `127.0.0.1:4444`) using **WebdriverIO** as the test client. Matches the Tauri v2 official E2E pattern (https://v2.tauri.app/develop/tests/webdriver/example/webdriverio/). The original D-27.15 named Playwright, but research (`27-RESEARCH.md` §Standard Stack and §Open Question 1) established that `tauri-driver` 2.0.6 speaks WebDriver classic which Playwright does NOT speak — Playwright over `tauri-driver` is not a viable wire-protocol combination. WebdriverIO is the protocol-compatible Tauri-recommended client. Slower job (~3–5 min on the GitHub Ubuntu runner) but tests the actual Tauri runtime + IPC layer + React frontend, NOT a mocked Vite dev server with stubbed `invoke()` calls.
  - **Why amended:** preserve the SPIRIT of D-27.15 (real binary + real IPC + Ubuntu only) over the LETTER (the tool name). The two rejected alternatives — Playwright + Vite dev server (skips IPC) and `tauri-plugin-playwright` 0.1.0 (single-maintainer plugin that contradicts the "no `hp41-gui/src-tauri/` source changes" invariant) — fail the same criteria D-27.15 originally rejected "dev server via Vite" against.
  - **Devdeps added:** `webdriverio` + `@wdio/cli` + `@wdio/local-runner` + `@wdio/mocha-framework` + `@wdio/spec-reporter` (all `^9.19`) in `hp41-gui/devDependencies`; `tauri-driver` crate (2.0.6) via `cargo install tauri-driver --locked` in CI; `webkit2gtk-driver` apt package (in addition to the already-present `libwebkit2gtk-4.1-dev`).
  - **Job runtime budget:** the cargo-install step is the slowest leg; cache `tauri-driver` via `actions/cache` keyed on the binary hash (see RESEARCH.md §Pitfalls #7). Expected end-to-end job ~3–5 min once cached, ~6–8 min cold.

- **D-27.16: Required job with 1 retry.** Mark the E2E job required for merge in `ci-gui.yml`. Configure `wdio.conf.js` with `mochaOpts.retries = 1` (Mocha retry semantics) so transient WebKitGTK / X server startup hiccups don't fail the PR, but a real regression (e.g. broken dispatch path) still does (retry would also fail). Standard pattern.
  ...
  - **Note:** D-27.16 originally named `playwright.config.ts`; the retries surface moves to `wdio.conf.js` `mochaOpts.retries: 1` under the D-27.15 amendment. Semantics unchanged.
...
- **`tauri-driver` apt package selection.** Tauri 2.11 ships `tauri-driver` as a crate (`cargo install tauri-driver --locked`); the Ubuntu runner needs the `webkit2gtk-driver` apt package in addition to the existing `libwebkit2gtk-4.1-dev`. Planner confirms whether `xvfb-run` wrapper is needed (GitHub `ubuntu-latest` typically has Xvfb pre-installed but `tauri-driver` may need a `DISPLAY=:99` env or an explicit `Xvfb :99 &` step). [Per D-27.15 AMENDED.]
- **E2E test file location.** `hp41-gui/e2e/smoke.spec.ts` (separate from Vitest unit tests in `hp41-gui/src/*.test.tsx`); `hp41-gui/wdio.conf.js` for WebdriverIO configuration (NOT `playwright.config.ts`). Planner finalizes the directory layout.
- **`data-testid` addition on `<Display14Seg />`.** Per RESEARCH.md Pitfall 10, the LCD component needs `data-testid="lcd-display"` so the WebdriverIO spec can locate it. The keyboard already has `data-key-id` on every key (Keyboard.tsx:285, 303). This one-line edit IS allowed under the "no source changes to hp41-gui/src-tauri/" invariant because `Display14Seg.tsx` lives in `hp41-gui/src/`, not `hp41-gui/src-tauri/`. Planner schedules this edit in the E2E plan.
...
- `hp41-gui/e2e/smoke.spec.ts` — NEW (D-27.13, FN-QUAL-05) — WebdriverIO + Mocha spec for `2 ENTER 3 + → 5.0000` [D-27.15 AMENDED: WDIO not Playwright]
- `hp41-gui/wdio.conf.js` — NEW — WebdriverIO config pointing at `tauri-driver` (`127.0.0.1:4444`) with `mochaOpts.retries = 1` (D-27.16)
- `hp41-gui/src/Display14Seg.tsx` — ADD `data-testid="lcd-display"` (RESEARCH.md Pitfall 10) — one-line edit, allowed under "no `src-tauri/` changes" invariant
- `hp41-gui/package.json` — add `webdriverio`, `@wdio/cli`, `@wdio/local-runner`, `@wdio/mocha-framework`, `@wdio/spec-reporter` (`^9.19`) to `devDependencies`
