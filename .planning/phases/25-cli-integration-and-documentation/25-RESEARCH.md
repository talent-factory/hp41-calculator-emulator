...
4. **What happens when user presses `f` while a `RegisterPrompt` modal is open but `shift_armed == false`?** Options:
   ...
   **Recommendation:** (b) — the modal IS the active dialog; `f` semantics inside a modal mean "modify this address with IND" per hardware. Document this as the canonical interpretation in CLAUDE.md. **RESOLVED:** Implemented as option (a)+(b) hybrid via reuse of `App.shift_armed`: pressing `f` inside an open modal arms `shift_armed=true` (same one-shot bit as the top-level prefix); the next `0` toggles the modal's `ind` field and consumes `shift_armed`. Plan 02 (Task 2) is the implementation site. Honors Pitfall 10 + D-25.12 verbatim while avoiding a parallel `shift_pending` field.

5. **Should `pending_prompt()` show `f→` when `shift_armed` is true even without an active modal?** The status bar would read `f→ Ready` instead of `Ready`. Mild UX improvement; trivial implementation. **Recommendation:** Yes. **RESOLVED:** Plan 01 (Task 2) wires the `f→` indicator into `render_status` (bottom-right status slot, shown when `app.shift_armed && app.pending_input.is_none() && !app.state.alpha_mode`).

6. **`docs/keyboard-layout.md` — rewrite or delete?** Currently a v1.x schematic with letter-binding conventions. Phase 25 makes it obsolete. **Recommendation:** Rewrite to point at `docs/hp41cv-function-matrix.md` and include a small ASCII-art keyboard with f-shifted labels in parentheses (3 mins of work). Delete is also acceptable per D-25.17 README updates. **RESOLVED:** Plan 04 (Task 2) ships the canonical generated `docs/hp41cv-function-matrix.md` via a standalone non-workspace Rust bin under `scripts/docs-matrix/` (driven by `just docs-matrix`); `docs/keyboard-layout.md` is superseded and may be rewritten/deleted as a follow-on housekeeping pass — not gated on Phase 25 close.
