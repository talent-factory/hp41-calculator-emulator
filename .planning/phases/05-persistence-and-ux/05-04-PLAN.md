...
must_haves:
  truths:
    - "HELP_DATA contains entries grouped by 10 categories covering all ~130 HP-41 operations"
  ...
  artifacts:
    - path: "hp41-cli/src/help_data.rs"
      provides: "HELP_DATA static array with all ~130 entries in 10 category groups"
...
Output: `help_data.rs` with HELP_DATA const containing ~130 entries across 10 categories; `programs.rs` with SampleProgram struct, sample_programs() accessor, and 10 HP-41 programs as Vec<Op>.
</objective>
...
<task type="auto" tdd="true">
  <name>Task 1: Create help_data.rs with HELP_DATA static array (~130 entries, 10 categories)</name>
  ...
  <behavior>
    - HELP_DATA length >= 130 (counting category header rows)
  ...
  //! ~130+ entries covering all keyboard-accessible HP-41 operations.

  /// (key_binding, hp41_op_name, description) — ~130+ entries, 10 categories.
  ...
  mod tests {
      ...
      fn test_help_data_has_minimum_entries() {
          // At least 130 entries (including category headers)
          assert!(
              HELP_DATA.len() >= 50,
