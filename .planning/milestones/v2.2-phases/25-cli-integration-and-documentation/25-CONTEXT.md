...
### Decision added during plan revision (W5 fix, 2026-05-14)

- **D-25.18: KEY_REF_TABLE is JSON-derived, not a parallel hand-curated list.** FN-CLI-01's "explicit KEY_REF_TABLE entries" requirement is satisfied by `hp41-cli/src/help_data.rs::help_entries()` rendering every JSON entry from `docs/hp41cv-functions.json` with `key_path != null` as a discoverability row. A parallel hand-curated `KEY_REF_TABLE` in `keys.rs` is FORBIDDEN — it would be a second source of truth and contradicts D-25.16 (canonical JSON pipeline). Drift between bindings and discovery is prevented by the same CI parity test family that guards D-25.15 (bidirectional Op-enum ↔ JSON drift catch in `function_matrix_parity.rs`), supplemented by the new `key_coverage.rs` test in Plan 04 Task 4 which asserts every implemented JSON entry with non-null `key_path` dispatches via `key_to_op` / `shifted_key_to_op` / modal-opener / `xeq_by_name_local_resolve` to a known `Op::` variant. Rationale: Phase 25's "explicit table" wording (FN-CLI-01) was inherited from the pre-Phase-25 v1.x crossterm direct-binding era; Phase 25's prefix-shift architecture (D-25.1..D-25.7) rendered a parallel table redundant. The reinterpretation is auditable via Plan 01's must_haves (which now cite D-25.18) and Plan 04 Task 3 (KEY_REF_TABLE regenerated-or-deleted decision, also cited to D-25.18).
