...
- v3.0 Math Pac I Emulation — IN PROGRESS (Phases 28–32)
  ...
  - Phase 32 Test Hardening & Quality Gates (2026-05-18) — `tests/` + `scripts/` + `.github/` + `justfile` only; 3 plans; meta-gate graduation (`math1_op_test_count` + `xrom_shadowing` actively cross-check 45 Op variants × 14 test files + 52 MATH_1.ops × 18-entry allowlist); `lint_math1_assertions.rs` Pitfall 14 + 17 discipline; `numerical_accuracy.rs` 566 → 763 cases (99.3 % pass); E2E smoke extended (`sinh(1)` + `MATRIX DET` Math Pac I workflows on Ubuntu); `scripts/check-free42-contamination.sh` D-32.7 12-symbol guard in `just ci` + `ci.yml::license-audit` parallel job (D-32.8). Coverage gate 91.74 % lines / 92.14 % regions REMAINS below the 95 % / 93 % threshold inherited from v2.2 — README hard-claim graduation DEFERRED to v3.0.1 per D-30.9 gate-conditional discipline.
...
*Stand 2026-05-18: Phases 28–32 shipped (26/26 plans). Phase 32 (Test Hardening) closes the v3.0 test/CI infrastructure: meta-gate graduation, `lint_math1_assertions.rs`, `numerical_accuracy.rs` extended to 763 cases at 99.3 % pass rate, E2E smoke extended with `sinh(1)` + `MATRIX DET` Math Pac I workflows on Ubuntu, Free42 GPL-contamination guard in BOTH `just ci` AND a dedicated `ci.yml::license-audit` parallel job. Coverage gate ≥ 95 % UNMET (91.74 % lines / 92.14 % regions) — README v3.0 hard-claim graduation DEFERRED to v3.0.1 per D-30.9 gate-conditional discipline; the test/CI infrastructure ships green but the OM-cited "feature-complete per Owner's Manual 00041-90034" claim waits on a per-file error-branch coverage push (poly.rs 76.37 %, trans.rs 81.17 %, four.rs 81.29 %, mod.rs 56.25 %, solve.rs 85.77 %, difeq.rs 85.76 %, integ.rs 90.86 %, matrix.rs 89.68 % all below the 90 % per-file floor). Scope remains Math 1 Pac only (XROM-Framework + Matrix/Komplex/Polynom/Integration/Solver/Vektor). Stat 1 → v3.1.*
...
*Last updated: 2026-05-18 — v3.0 Phase 32 (Test Hardening & Quality Gates) shipped (3/3 plans). Meta-gate graduation (`math1_op_test_count` + `xrom_shadowing` non-vacuous over 45 Math Pac I variants + 52 MATH_1.ops mnemonics); `tests/lint_math1_assertions.rs` Pitfall 14 + 17 assertion discipline; `numerical_accuracy.rs` extended 566 → 763 cases at 99.3 % combined pass rate with D-27.6 baseline floor preserved bit-for-bit; E2E smoke gained `XEQ "SINH" 1 → 1.1752` (xrom_resolve) + `XEQ "MATRIX" 2x2 DET → -2.0000` (modal pipeline) on Ubuntu via `ci-gui.yml::e2e-linux`; Free42 GPL-contamination guard (D-32.7 12-symbol policy with disclaim-header allowlist) wired into `just ci` AND `ci.yml::license-audit` parallel job per D-32.8 belt+suspenders. Coverage gate ≥ 95 % UNMET (91.74 % lines / 92.14 % regions); README v3.0 hard-claim graduation DEFERRED to v3.0.1 per D-30.9 gate-conditional discipline.*
