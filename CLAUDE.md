...
### v3.0 additions (Math Pac I Emulation, Phases 28–32 — README hard-claim DEFERRED pending coverage gate)
...
#### Phase 32 (Test Hardening & Quality Gates, shipped 2026-05-18 — README hard-claim DEFERRED)
...
- **Coverage gate ≥ 95 % UNMET — README hard-claim graduation DEFERRED (Rule 4):** `just coverage` reports 91.74 % lines / 92.14 % regions on the v3.0 baseline (below the 95 % / 93 % thresholds inherited from v2.2). Multiple `ops/math1/*.rs` files sit below the 90 % per-file floor (poly.rs 76.37 %, trans.rs 81.17 %, four.rs 81.29 %, mod.rs 56.25 %, solve.rs 85.77 %, difeq.rs 85.76 %, integ.rs 90.86 %, matrix.rs 89.68 %), plus `ops/program.rs` 86.42 %. Plan 32-02's bulk numerical_accuracy.rs extension exercises existing math1 source via `dispatch()` but did not materially move per-file coverage because the gaps live in error-branch / edge-case arms not reachable through the OM-cited happy-path cases. Per the D-30.9 / D-32.5 gate-conditional discipline (mirroring v2.2 HP-41CV claim graduation), the README v3.0 line REMAINS at the soft-claim "10 top-level programs, ~55 XEQ entry points, documented divergences" wording. Graduation to the OM-cited hard claim "v3.0 ships Math Pac I behavioral emulation, feature-complete per Owner's Manual 00041-90034" is DEFERRED to a follow-up milestone (likely v3.0.1) once the per-file coverage gap is closed — either via a targeted error-path test pass on the 7 below-90 % math1 files, or via documenting acceptable per-file thresholds in an ADR raising the gap to the OM-divergence catalog. The test/CI infrastructure SHIPS green (lint + test + license-audit + E2E smoke); only the README claim graduation waits.
