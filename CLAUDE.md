...
- **Post-graduation polish before v3.0 ship (2026-05-20, quick-task batch on `develop`):** six post-32-10 commits address visual/UX issues surfaced during the v3.0 release readiness review — none are formally GSD-planned phases (they ship as a quick-task batch analogous to v2.1's reconcile-by-quick-tasks pattern). (1) `2b547b9` fix MSRV-CI `uninlined_format_args` on 5 sites (`hp41-core/src/ops/program.rs:350` + 4 assert! messages — pre-existing failure from Plan 31-04, surgical `hp41-core` exception under the lint-only / no-behavior-change rule). (2) `ca87222` cargo fmt cleanup on 8 Wave 1 test files (subagent-worktree drift). (3) `555eb8f` E2E `XEQ "SINH" 1` + `XEQ "MATRIX" 2x2 DET` smoke tests assert on the `dispatch_op` response's `CalcStateView.display_str` instead of `[data-testid="lcd-display"]` data-text — the prior assertion path was incompatible with `__TAURI_INTERNALS__.invoke` because the frontend deliberately does not poll for state updates (D-11 — no polling) and an out-of-band invoke does not propagate to the React tree. (4) `fe33be3` right-panel `key_ref_entries()` gains an `entry.xrom.is_none()` filter — Phase 29 SC-4 / D-29.2 / CLI-04 acceptance criteria are formally SUPERSEDED because the ~45 `XEQ "..."` rows crowded out the actual keyboard reference; module functions remain fully discoverable via the `?` overlay's "Math 1 Pac (XROM 7)" section. (5) `d3c68c5` standard ratatui modal pattern — `frame.render_widget(Clear, overlay_area)` before `render_stateful_widget` on both `?` help and Ctrl+P programs overlays — fixes right-panel/stack/display bleed-through visible as e.g. `XEQ "X<Y?"X<Y?` appearing inside the help table's Math section. (6) `201b3f7` incremental substring search in the `?` overlay (`App.help_search_query`, `help_data::filter_help_rows`, 3-line search bar above the table, ↑↓/PgUp/PgDn navigation, `q`/`j`/`k` legacy nav suppressed once query is non-empty) — mirrors hp41-gui's `HelpOverlay.tsx` search input, prepares the overlay for v3.1 Stat / v3.2 Time / v3.3 Advantage Pacs which will push the row count past 200. 12 new tests landed (6 in `help_data::tests`, 6 in `app::tests`). The v3.0 release ships at the cumulative HEAD of these six commits.
