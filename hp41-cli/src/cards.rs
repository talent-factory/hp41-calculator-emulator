...
//!   and phase 3 (apply read result). The hp41-gui Tauri thunk needs the
//!   split so disk I/O does not hold the AppState mutex.
//! - `drain_pending_card_op()` — single-call convenience wrapper used by
//!   the single-threaded CLI event loop.
//!
//! SC-4 invariant: this module calls only the public `hp41_core::cardreader::*`
//! API for encoding/decoding. No codec logic lives here.
//!
//! SYNC-NOTE: keep this file in step with `hp41-gui/src-tauri/src/cards.rs`.
...
/// caller's exclusive access to `state`. Used by the single-threaded CLI
/// event loop — the GUI thunk uses the phases separately so it can release
/// the AppState mutex around the I/O step.
